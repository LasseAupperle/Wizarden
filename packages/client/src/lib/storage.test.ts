...
describe('storage (persistence §18)', () => {
  it('defaults settings to dark + english + sound on', () => {
    expect(storage.getSettings()).toEqual({ sound: true, animations: true, theme: 'dark', language: 'en' });
  });

  it('persists and reloads settings', () => {
    storage.setSettings({ sound: false, animations: true, theme: 'light', language: 'nl' });
    expect(storage.getSettings()).toEqual({ sound: false, animations: true, theme: 'light', language: 'nl' });
  });
